...
ALIGN(512) uint8_t rd_buffer[0x200];
ALIGN(512) uint8_t sector_buffer[5000];

volatile uint32_t last_wr_sector = 0xffffffff;
...
{
	FLASH_read(offset, length, rd_buffer);
	*buff_adr = rd_buffer;
...
{
	volatile uint32_t wr_sector = offset & 0xfffff000;

	if (last_wr_sector != wr_sector) {
		if (last_wr_sector != 0xffffffff) {
			FLASH_write_enable();
			FLASH_sector_erase(last_wr_sector);
			FLASH_wait_busy();

			FLASH_write_from_ram(sector_buffer, last_wr_sector, 0x10);
		}

		FLASH_wait_busy();
		FLASH_read_sector(wr_sector, sector_buffer);
	}

	//if (memcmp(&sector_buffer[offset & 0xfff], *buff_adr, length)) {
	memcpy(&sector_buffer[offset & 0xfff], *buff_adr, length);
	//}
